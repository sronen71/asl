...
def process_results(sequence_results,sequence_id):
    ...
    for i,results in enumerate(sequence_results):
        ...
        pose_landmarks=results.pose_landmarks
        right_hand_landmarks=results.right_hand_landmarks
        parts={"face":face_landmarks,"left_hand":left_hand_landmarks,"pose":pose_landmarks,"right_hand":right_hand_landmarks}
        parts_one_row=[]
        for part in parts:
            ...
            #print(part,len(data))
            df_part_one_row = pd.DataFrame([data], columns=[f"{c}_{part}_{i}" for c in columns for i in range(len(df))])
        ...
        new_columns=["sequence_id","frame"]+[col for col in columns if 'x_' in col]+[col for col in columns if 'y_' in col]+[col for col in columns if 'z_' in col]
...
def main():
    ...
    if "chicago.csv" in glob.glob("*.csv"):
        ...
        df_done=None
        num_done=0
    data1=pd.read_csv(path1+"/"+path1+".csv",converters={"signer":str}) #filename,label_proc
    data1["path"]=path1
    data2=pd.read_csv(path2+"/"+path2+".csv",converters={"signer":str}) #filename,label_proc
    data2["path"]=path2
